...
namespace ExpressCommon
{
    public class SQLHelper
    {
        ...
        /// 获取关系型数据库配置信息
        /// </summary>
        public static string defConnStr = ConfigurationManager.ConnectionStrings["MSSQL_Default"].ToString();
        ...
        /// <remarks>
        /// 示例:  
        /// int result = ExecuteNonQueryTran(connString, CommandType.StoredProcedure, "PublishOrders", new SqlParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connectionString">一个有效的数据库连接字符串</param>
        /// <param name="commandType">命令类型 (存储过程,命令文本, 其它.)</param>
        /// <param name="commandText">存储过程名称或SQL语句</param>
        /// <param name="commandParameters">SqlParameter参数数组</param>
        /// <returns>返回命令影响的行数</returns>
        public static int ExecuteNonQueryTran(string connectionString, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            // ExecuteNonQueryTran #1
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    //执行数据库操作
                    int val = ExecuteNonQuery(transaction, commandType, commandText, commandParameters);
