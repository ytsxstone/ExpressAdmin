...
namespace ExpressCommon
{
    ...
    public class ExcelHelper
    {
        ...
        /// <param name="filePath">Excel文件物理路径（带文件名）</param>
        /// <returns></returns>
        public static DataTable ExportToDataTable_New(string filePath)
        {
            DataTable dt = new DataTable();
            if (File.Exists(filePath))//获取文件的后缀名
            {
                ...
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    object clomnname = sheet.Cells[1, i].Value;
                    dt.Columns[i].ColumnName = clomnname != null ? clomnname.ToString() : ("ColumnName" + i);
                }
                return dt;
            }
        ...
        /// <param name="saveFileType">保存文件类型（.xls;.xlsx）</param>
        /// <returns>文件物理路径</returns>
        public static string SaveDataTableToExcel_New(DataTable dt, List<string> numberColumns, List<string> redStyleColumns, List<string> centerStyleColumns, List<string> contentLeftStyleColumns, Dictionary<string, double> dictWidthColumns, string filePath, string fileName, EnumExcelFileType saveFileType)
        {
            string savePath = Path.Combine(filePath, fileName);
            if (!Directory.Exists(filePath))
            {
